...
namespace BlackCoat.Animation
{
	...
	public static class Interpolation
	{
		...
		public static Func<float, float, float, float, float> Get(InterpolationType type)
		{
			...
			throw new Exception($"Unknown {nameof(InterpolationType)} {type}");
		}
		...
		{
			if (elapsedTime >= duration) return start + distance;
		...
		internal static float InOutExpo(float start, float distance, float elapsedTime, float duration)
		{
			...
			if (elapsedTime < 1) return (float)(distance / 2 * Math.Pow(2, 10 * (elapsedTime - 1)) + start);
			elapsedTime--;
		...
		{
			elapsedTime = (elapsedTime > duration) ? 2.0f : elapsedTime / (duration / 2);
			if (elapsedTime < 1) return (float)(-distance / 2 * (Math.Sqrt(1 - elapsedTime * elapsedTime) - 1) + start);
			elapsedTime -= 2;
		...
		internal static float InElastic(float start, float distance, float elapsedTime, float duration)
		{
			...
			if (elapsedTime == 0.0f) return start;
			if ((elapsedTime /= duration) == 1.0f || duration == 0) return start + distance;
