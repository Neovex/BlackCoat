...
{
    public class ParticleRole : Role
    {
        // Variables #######################################################################
        // System
        ...
        public ParticleRole(Emitter emitter, Core core):base(core)
        {
        ...
        public override void Update(Single deltaT)
        {
            if (Alive)
            {
                ...
                pos.X += (float)(Math.Cos(_Direction * Constants.DEG_TO_RAD) * _Speed * deltaT);
                pos.Y += (float)(Math.Sin(_Direction * Constants.DEG_TO_RAD) * _Speed * deltaT);
                Target.Position = pos;

                //Blending
                Target.Alpha += _Blend * deltaT;

                //Rotation
                Target.Rotation += _RotationRate * deltaT;

                //Scaling
                var scale = Target.Scale; //check
